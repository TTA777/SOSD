...
function do_benchmark() {
    ...
    if [ -f $RESULTS ]; then
        ...
        vtune -collect uarch-exploration $BENCHMARK -r $2 ./data/$1 ./data/$1_equality_lookups_10M --pareto --only=$3 | tee ./results/$1_results.txt
    fi
}
...
function do_benchmark_csv() {
    ...
    vtune -collect uarch-exploration $BENCHMARK -r $2 ./data/$1 ./data/$1_equality_lookups_10M --pareto --csv  --only=$3
}
