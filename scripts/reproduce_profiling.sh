...
for iteration in $(seq 1 $num_iterations); do
  ...
  for write_protion in 0 100; do
    ...
    qsub -d . -l nodes=1:gold6128:ppn=2 ./scripts/execute_profiling.sh -F "-w $write_protion -i $iteration"
    done
  # Mixed workloads
  for write_protion in 25 50 75; do
    ...
    qsub -d . -l nodes=1:gold6128:ppn=2 ./scripts/execute_profiling.sh -F "-w $write_protion -i $iteration -d scripts/datasets_under_test_write.txt"
    done
done
